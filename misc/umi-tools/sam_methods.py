...
class get_bundles:
    ...
    def __call__(self, inreads):
        ...
        intron_gene_tag = "GI:Z:"
        intron_assigned_tag = "IS:Z:"
        exon_gene_tag = "GE:Z:"
        exon_assigned_tag ="IS:Z:"
        ...
        for read in inreads:
            ...
            if self.options.per_gene:
                ...
                if self.options.per_contig:
                    ...
                    assigned = ""
                    gene = ""
                    #if by intron assigned genes should be used -> check both tags in bam-file
                    if(self.options.use_introns):
                        
                        if read.has_tag(exon_gene_tag):
                            assigned = read.get_tag(exon_assigned_tag)
                            gene = read.get_tag(exon_gene_tag)
                        elif read.has_tag(intron_gene_tag):
                            assigned = read.get_tag(intron_assigned_tag)
                            gene = read.get_tag(intron_gene_tag)
                        else:
                            self.read_events['Read skipped, no assigned gene'] += 1
                            continue
                    else:
