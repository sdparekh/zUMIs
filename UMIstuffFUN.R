...
prep_samtools <- function(featfile,bccount,inex,cores,samtoolsexc){
  ...
  samcommand <- paste(samtoolsexc," view -x BX -x NH -x AS -x nM -x HI -x IH -x NM -x uT -x MD -x jM -x jI -x XN -x XS -x ES -x EN -x IS -x IN -x UG -x",UMItagtrim,layoutflag,"-@")
  #grepcommand <- " | cut -f12,13,14 | sed 's/BC:Z://' | sed 's/G[E|I]:Z://g' | sed 's/XX:Z://' | grep -F -f "
  ...
  if(inex == FALSE){
    grepcommand <- " | cut -f12,13,14 | sed 's/BC:Z://' | sed 's/GE:Z://' | sed 's/XX:Z://' | grep -F -f "
...
umiFUNs<-list(umiCollapseID=umiCollapseID,  umiCollapseHam=NA)
...
fixMissingOptions <- function(config){
  ...
  if(is.null(config$python_exec)){
    get_py <- system("which python", intern = TRUE)
    config$python_exec <- get_py[1]
  }
...
umi_tools_collapse <- function(opt, bamfile){
...
testModules <- function(pythonExec){
  require(reticulate)
  package <- tryCatch(
    {
      use_python(pythonExec)
      message("Looking for Python packages needed for UMI-tools...")
      import("regex")
      import("numpy")
      import("pysam")
    ...
    error=function(cond){
      message(paste("Could not import all needed python packages:"))
      message(paste(cond,"\n"))
      return(FALSE)
    })
  return (TRUE)
