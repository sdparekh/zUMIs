...
## Default parameters ##
	...
	-b  <Barcodes>           : Either number of cell/sample barcodes to output (in case of Drop-seq - e.g. 100)
				   or defined barcodes as a text file (e.g. ATGCCAAT).  Default: 100.
...
while getopts ":R:f:r:g:o:a:t:s:c:m:l:b:n:q:Q:z:u:x:e:p:i:d:h" options; do #Putting <:> between keys implies that they can not be called without an argument.
  case $options in
  R ) isslurm=$OPTARG;;
  f ) bcread=$OPTARG;;
  r ) cdnaread=$OPTARG;;
...
if [[ "$isslurm" == "yes" ]] ; then
	...
	bash $zumisdir/zUMIs-mapping.sh $sname $outdir $genomedir $gtf $threads $readlen "$starparams" $starexc $samtoolsexc
	bash $zumisdir/zUMIs-prepCounting.sh $sname $outdir $threads $samtoolsexc
	bash $zumisdir/zUMIs-counting.sh $sname $outdir $barcodes $threads $gtf $strandedness $xcrange $xmrange $subsampling $zumisdir
	bash $zumisdir/zUMIs-cleaning.sh $sname $outdir
else
	bash $zumisdir/zUMIs-noslurm.sh  $bcread $cdnaread $sname $outdir $xcrange $xmrange $cbasequal $mbasequal $molbcbase $cellbcbase $threads $genomedir $gtf $readlen "$starparams" $starexc $barcodes $strandedness $subsampling $zumisdir $samtoolsexc
fi
